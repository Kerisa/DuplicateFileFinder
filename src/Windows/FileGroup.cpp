...
std::wstring Filter::MakeCompareCommand() const
{
...
std::vector<std::wstring> Filter::MakeSearchCommand() const
{
  ...
  {
    cmds.push_back(std::wstring(L"Finder.exe \"") + it.mDir + L"\"");
    if (!cmd.empty())
      cmds.back() += L" " + cmd;
  }
  return cmds;
...
size_t Filter::GetSearchPathOrder(const std::wstring& p) const
{
  for (size_t i = 0; i < mSearchDirectory.size(); ++i) {
    if (p.find(mSearchDirectory[i].mDir) != std::wstring::npos)
      return i;
  }
  return mSearchDirectory.size();
}
